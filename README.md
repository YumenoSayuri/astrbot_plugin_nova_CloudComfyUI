...
- 改图未指定分辨率时按原图比例自动放大或缩小到单边不超过 1980
...
如果当前消息、引用消息中带图，或显式提供了 `--image_source`，插件会自动提取该图 URL，并默认切换到编辑模型 `14`。如果没有手动指定分辨率，还会按原图宽高比自动放大或缩小到单边不超过 `1980`。
...
当前版本：[`v1.4.4`](AstrBot/data/plugins/astrbot_plugin_nova_mengyudraw/metadata.yaml:4)